...
entries in your world, and needs nothing else installed.
...
- **Answers cited to the entry they came from**, so you can click through and
  check rather than take the archivist's word for it.
- **Your players' permissions are respected.** Lite reads Foundry's own journal
  permissions, so a GM-only note stays GM-only even when a player asks about it.
...
Lite reads one journal folder and can only hold as much as fits in a single
question. Vault reads everything you have.

| | **Lite** *(this module alone)* | **Tusk's Vault** |
|---|---|---|
| To install | Nothing | A free download |
| Where your lore lives | Journal entries in one folder | The notes you already keep — Word, PDF, Markdown, or a whole Obsidian vault |
